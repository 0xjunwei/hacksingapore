...
async function main() {
  ...
  const poapAddress = "0x9e86756c2041474DDe8f484b17032bEFC59ccB2e";
  ...
  // signing up wallet
  const signupWallet = new ethers.Wallet(process.env.SIGNER_WALLET, provider);
  ...
  //sign up first
  const signUpTx = await poapContract.connect(signupWallet).signUp();
  await signUpTx.wait();
  ...
  // Stop sign up
  const stopSignUpTx = await poapContract.toggleSignUp();
  await stopSignUpTx.wait();
  console.log("Stopped users from signing");
}
